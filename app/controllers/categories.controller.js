...
exports.create = {
    ...
    validate: [
        check('name')
            ...
            .matches(/^[\D ]+$/).withMessage(strings.CATEGORY_NAME_MATCHES),
...
exports.join = {
    ...
    validate: [
        check('columnName')
            .isAlpha().withMessage(strings.CATEGORY_COLUMN_NAME_ALPHA),
    ...
    inDatabase: (req, res, next) => {
        ...
        return database.sequelize.transaction((t) => {
            return Categories.findAll({where: {[Op.or]: ids}}, {transaction: t});
        }).then(data => {
            if (data.length > 0 || data !== undefined) {
                return res.status(200).json(data);
            } else {
